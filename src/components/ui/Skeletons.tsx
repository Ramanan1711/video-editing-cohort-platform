    ...
    <div className="space-y-6 animate-pulse" aria-busy="true" aria-label="Loading dashboard content">
      ...
      <div className={`grid gap-4 sm:grid-cols-2 lg:grid-cols-${cardsCount}`}>
        {Array.from({ length: cardsCount }).map((_, i) => (
          <div
            key={i}
            className="rounded-2xl border border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900 p-5 shadow-2xs"
          >
      ...
      {showChart && (
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-3xl border border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-2xs lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="h-5 w-40 rounded-md bg-slate-200 dark:bg-slate-800" />
              <div className="h-8 w-24 rounded-xl bg-slate-100 dark:bg-slate-800/60" />
            </div>
            <div className="h-64 rounded-2xl bg-slate-100/70 dark:bg-slate-800/40" />
          </div>
          <div className="rounded-3xl border border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-2xs space-y-4">
            <div className="h-5 w-32 rounded-md bg-slate-200 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-800 pb-4" />
            <div className="space-y-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex items-center gap-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 p-3">
  ...
  return (
    <div className="space-y-6 animate-pulse" aria-busy="true" aria-label="Loading review queue">
      {[1, 2].map((i) => (
        <div
          key={i}
          className="rounded-3xl border border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm space-y-5"
        >
          {/* Header Row */}
          ...
          <div className="rounded-2xl bg-slate-900/10 dark:bg-slate-950/60 p-4 space-y-3">
            ...
            <div className="h-10 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700" />
          </div>

          {/* Rubric Grid */}
          <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 p-4 space-y-3">
            <div className="h-4 w-44 rounded bg-slate-200 dark:bg-slate-700" />
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              {[1, 2, 3, 4, 5].map((s) => (
                <div key={s} className="h-20 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2.5" />
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <div className="h-9 w-32 rounded-xl bg-orange-200 dark:bg-orange-950/60" />
          </div>
        </div>
      ))}
    </div>
...
export function TableSkeleton({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs animate-pulse" aria-busy="true">
          ...
          <div key={r} className="flex items-center justify-between px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2.5 w-44 rounded bg-slate-100 dark:bg-slate-800" />
              </div>
            </div>
            <div className="h-6 w-20 rounded-full bg-slate-100 dark:bg-slate-800" />
            <div className="h-4 w-16 rounded bg-slate-100 dark:bg-slate-800 hidden sm:block" />
...
export function CurriculumSkeleton() {
  return (
    <div className="space-y-4 animate-pulse" aria-busy="true">
      {[1, 2, 3].map((m) => (
        <div key={m} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs space-y-3">
          ...
          <div className="space-y-2">
            {[1, 2, 3].map((l) => (
              <div key={l} className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-slate-800/50 p-3">
                <div className="flex items-center gap-3">
                  <div className="size-6 rounded-md bg-slate-200 dark:bg-slate-700" />
...
export function CalendarSkeleton() {
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm animate-pulse" aria-busy="true">
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
      ...
      <div className="grid grid-cols-7 gap-2">
        {Array.from({ length: 35 }).map((_, i) => (
          <div key={i} className="h-16 rounded-xl bg-slate-50 dark:bg-slate-800/50 p-2 space-y-1">
            <div className="h-3 w-4 rounded bg-slate-200 dark:bg-slate-700" />
            {i % 4 === 0 && <div className="h-2 w-full rounded bg-orange-200 dark:bg-orange-950/60" />}
          </div>
        ))}
      </div>
    </div>
